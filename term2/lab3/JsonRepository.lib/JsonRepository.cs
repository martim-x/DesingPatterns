...
public class JsonRepository : ICelebrity<Celebrity>
{
    public static string JSONFileName = "Celebrities.json";

    public string BasePath { get; private set; }

    public string FullBasePath => Path.Combine(this.BasePath, JsonRepository.JSONFileName);

    private List<Celebrity> _celebrities = new();
    private int _scn = 0;

    private JsonRepository(string basePath)
    {
    ...
    {
        return new JsonRepository(basePath);
    }
    ...
    public List<Celebrity> GetAll() => this._celebrities;

    public Celebrity? GetById(int id) => this._celebrities.FirstOrDefault(c => c.Id == id);
